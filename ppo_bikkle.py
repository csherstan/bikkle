...
from model import BikkleActionValueFunction, BikklePolicy, preprocess_bikkle_observation_with_mask, BikkleValueFunction
...
class BikkleAgent(nn.Module):
    def __init__(self, observation_space: gym.spaces.Dict, action_space: gym.spaces.Box, device=None):
        super().__init__()
        self.device = device
        self.value_function = BikkleValueFunction(observation_space).to(device)
        self.policy = BikklePolicy(observation_space, action_space).to(device)

    def get_value(self, x: dict[str, dict[torch.Tensor]]) -> torch.Tensor:
        """
        Computes the value of the given observation.

        :param x: A dictionary containing preprocessed observations.
        :return: A tensor representing the value.
        """
        return self.value_function(x)

    def get_action_and_value(
        self, obs: dict[str, dict[torch.Tensor]], action: Optional[torch.Tensor] = None
    ) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Computes the action, log probability, entropy, and value for the given observation.

        :param obs: A dictionary containing preprocessed observations.
        :param action: Optional tensor representing a specific action.
        :return: A tuple containing the action, log probability, entropy, and value.
        """
        tokens, indices, mask = obs["tokens"], obs["indices"], obs["mask"]

        # Use BikklePolicy.get_action to compute action, log probability, and mean
        action, log_prob, mean, entropy = self.policy.get_action(tokens, indices, mask, greedy=(action is not None))

        # Compute value using BikkleValueFunction
...
def rollout(obs, done, obs_space, device, avg_returns=[]):
    ts = []
    for step in range(args.num_steps):
        # ALGO LOGIC: action logic
        pp_obs = preprocess_bikkle_observation_with_mask(obs, observation_space=obs_space, max_blocks=4, device=device)
        action, logprob, _, value = policy(obs=pp_obs)
        ...
        obs["agent_position"] = torch.tensor(obs["agent_position"], dtype=torch.float32, device=device)
        obs["cyan"] = torch.tensor(np.array(list(obs["cyan"][0])))
        obs["pink"] = torch.tensor(np.array(list(obs["pink"][0])))
        obs["steps"] = torch.tensor(obs["steps"], dtype=torch.float32, device=device)
...
if __name__ == "__main__":
    ...
    def step_func(action: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return next_obs_np, torch.as_tensor(reward), torch.as_tensor(
            next_done), info


    ####### Agent #######
    agent = BikkleAgent(envs.single_observation_space, envs.single_action_space, device=device)
    # Make a version of agent with detached params
    agent_inference = BikkleAgent(envs.single_observation_space, envs.single_action_space, device=device)
    ...
    for iteration in pbar:
        ...
        next_obs, next_done, container = rollout(next_obs, next_done, obs_space=envs.single_observation_space,
                                                 device=device, avg_returns=avg_returns)
        global_step += container.numel()

        pp_next_obs = preprocess_bikkle_observation_with_mask(next_obs, observation_space=envs.single_observation_space,
                                                             max_blocks=4, device=device)
        container = gae(pp_next_obs, next_done, container)
        # TODO: I think if we have multiple environments I would need to figure this out
        container_flat = container.reshape(-1)
        ...
        if global_step % args.model_save_interval == 0:
            # Save models
            torch.save(agent.state_dict(), outdir / f"actor_model_{global_step}.pth")
